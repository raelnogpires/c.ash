...
import { Button, EmptyState, Icon, Modal, Sheet, TransactionList } from '../components'
...
export function TransactionsView({ data, transactions, occurrences, search, confirmOccurrence, dismissOccurrence, open, onImport, onEdit, onRemove, listTrashed, restoreTrashed, deletePermanently, emptyTrash }: { data: BootstrapData; transactions: Transaction[]; occurrences:TransactionOccurrence[];search(filter:TransactionFilter):Promise<Transaction[]>;confirmOccurrence(id:string):Promise<void>;dismissOccurrence(id:string):Promise<void>; open(): void; onImport(input: BankStatementInput): Promise<BankStatementImportResult>; onEdit(tx: Transaction, trigger: HTMLElement): void; onRemove(tx: Transaction, trigger: HTMLElement): void; listTrashed(): Promise<Transaction[]>; restoreTrashed(id: string): Promise<void>; deletePermanently(id: string): Promise<void>; emptyTrash(): Promise<void> }) {
  ...
  const [results,setResults]=useState<Transaction[]>(transactions),[filters,setFilters]=useState<TransactionFilter>({status:'active'}),[searching,setSearching]=useState(false),[searchError,setSearchError]=useState(''),[advancedOpen,setAdvancedOpen]=useState(false)
  useEffect(()=>setResults(transactions),[transactions])
  const runSearch=async(next:TransactionFilter)=>{setFilters(next);setSearchError('');setSearching(true);try{setResults(await search(next))}catch(err){setSearchError(errorMessage(err))}finally{setSearching(false)}}
  const fileInput = useRef<HTMLInputElement>(null), filterForm = useRef<HTMLFormElement>(null), advancedForm = useRef<HTMLFormElement>(null), advancedTrigger = useRef<HTMLButtonElement>(null), advancedValues = useRef<Record<string, string>>({})
  const readFilters = (): TransactionFilter => {
    const primary = filterForm.current ? new FormData(filterForm.current) : new FormData()
    const advanced = advancedForm.current ? new FormData(advancedForm.current) : new FormData()
    const value = (name: string) => advanced.has(name) ? advanced.get(name) : advancedValues.current[name] ?? primary.get(name)
    const minimum = parseBRL(String(value('minimum') ?? '')) ?? 0
    const maximum = parseBRL(String(value('maximum') ?? '')) ?? 0
    return {
      text: String(value('text') ?? ''), startDate: String(value('startDate') ?? ''), endDate: String(value('endDate') ?? ''),
      accountId: String(value('accountId') ?? ''), categoryId: String(value('categoryId') ?? ''), tag: String(value('tag') ?? ''),
      kind: String(value('kind') ?? '') as TransactionFilter['kind'], status: String(value('status') ?? 'active') as TransactionFilter['status'],
      minimumAmountCents: minimum, maximumAmountCents: maximum, recurrence: String(value('recurrence') ?? '') as TransactionFilter['recurrence'],
    }
  }
  const closeAdvanced = () => { if (searching) return; setAdvancedOpen(false); requestAnimationFrame(() => advancedTrigger.current?.focus()) }
  const rememberAdvanced = () => { if (!advancedForm.current) return; for (const [name, value] of new FormData(advancedForm.current).entries()) advancedValues.current[name] = String(value) }
  ...
  const clearFilter = (key: keyof TransactionFilter) => {
    ...
    const controls = [filterForm.current, advancedForm.current].map(form => controlName ? form?.elements.namedItem(controlName) : null)
    controls.forEach(control => { if (control instanceof HTMLInputElement || control instanceof HTMLSelectElement) control.value = key === 'status' ? 'active' : '' })
    if (controlName && key !== 'status') delete advancedValues.current[controlName]
    const next = { ...filters }
    delete next[key]
  ...
  return <div className="page transactions-page">
    ...
    {section === 'active' ? <div id="active-transactions" role="tabpanel" aria-labelledby="active-transactions-tab" tabIndex={0}>
      {searchError&&<div className="alert" role="alert">{searchError}<button type="button" onClick={()=>setSearchError('')} aria-label="Fechar aviso"><Icon name="close"/></button></div>}
      <form ref={filterForm} className="transaction-toolbar" aria-label="Filtros de movimentações" aria-busy={searching||undefined} onSubmit={event=>{event.preventDefault();void runSearch(readFilters())}}><div className="transaction-toolbar__primary"><label className="field"><span>Buscar</span><input name="text" placeholder="Descrição, conta ou categoria"/></label><label className="field"><span>Conta</span><select name="accountId"><option value="">Todas</option>{data.accounts.map(a=><option value={a.id} key={a.id}>{a.name}</option>)}</select></label><label className="field"><span>Tipo</span><select name="kind"><option value="">Todos</option><option value="income">Receita</option><option value="expense">Despesa</option><option value="transfer">Transferência</option></select></label><label className="field"><span>Tag</span><input name="tag" placeholder="Ex.: casa"/></label><label className="field"><span>Status</span><select name="status" defaultValue="active"><option value="all">Todos</option><option value="active">Realizadas</option><option value="pending">Pendentes</option></select></label></div><div className="transaction-toolbar__actions"><Button loading={searching}>Aplicar filtros</Button><Button ref={advancedTrigger} type="button" kind="secondary" onClick={()=>setAdvancedOpen(true)}>Mais filtros</Button><button type="button" className="text-button" onClick={()=>{filterForm.current?.reset();advancedForm.current?.reset();advancedValues.current={};void runSearch({status:'active'})}}>Limpar todos</button></div></form>
      {advancedOpen&&<Sheet onClose={closeAdvanced} busy={searching} labelledBy="transaction-filters-title" className="filter-sheet"><header><div><p className="context-label">Refinar resultados</p><h2 id="transaction-filters-title">Mais filtros</h2></div><button type="button" className="icon-button" aria-label="Fechar" disabled={searching} onClick={closeAdvanced}><Icon name="close"/></button></header><form ref={advancedForm} className="sheet-form filter-sheet__form" onSubmit={async event=>{event.preventDefault();rememberAdvanced();await runSearch(readFilters());closeAdvanced()}}><label className="field"><span>De</span><input name="startDate" type="date" defaultValue={advancedValues.current.startDate??''}/></label><label className="field"><span>Até</span><input name="endDate" type="date" defaultValue={advancedValues.current.endDate??''}/></label><label className="field"><span>Categoria</span><select name="categoryId" defaultValue={advancedValues.current.categoryId??''}><option value="">Todas</option>{data.categories.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select></label><label className="field"><span>Valor mínimo</span><div className="money-input"><span aria-hidden="true">R$</span><input name="minimum" inputMode="decimal" defaultValue={advancedValues.current.minimum??''}/></div></label><label className="field"><span>Valor máximo</span><div className="money-input"><span aria-hidden="true">R$</span><input name="maximum" inputMode="decimal" defaultValue={advancedValues.current.maximum??''}/></div></label><label className="field"><span>Recorrência</span><select name="recurrence" defaultValue={advancedValues.current.recurrence??''}><option value="">Todas</option><option value="recurring">Recorrentes</option><option value="nonrecurring">Não recorrentes</option></select></label><div className="form-actions"><Button type="button" kind="ghost" disabled={searching} onClick={closeAdvanced}>Cancelar</Button><Button loading={searching}>Aplicar filtros</Button></div></form></Sheet>}
