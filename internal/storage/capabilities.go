...
func (q *dbQueries) InsertTransactionOccurrence(ctx context.Context, x domain.TransactionOccurrence, at string) error {
	_, err := q.q.ExecContext(ctx, `INSERT INTO transaction_occurrences(id,recurrence_rule_id,account_id,kind,category_id,subcategory_id,amount_cents,description,scheduled_date,status,installment_number,installment_count,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, x.ID, nullable(x.RecurrenceRuleID), x.AccountID, x.Kind, nullable(x.CategoryID), nullable(x.SubcategoryID), x.AmountCents, x.Description, x.ScheduledDate, x.Status, x.InstallmentNumber, x.InstallmentCount, at, at)
	return err
}

const occurrenceSelect = `SELECT o.id,COALESCE(o.recurrence_rule_id,''),o.account_id,a.name,o.kind,COALESCE(o.category_id,''),COALESCE(c.name,''),COALESCE(o.subcategory_id,''),o.amount_cents,o.description,o.scheduled_date,o.status,COALESCE(o.transaction_id,''),o.installment_number,o.installment_count,o.created_at,o.updated_at FROM transaction_occurrences o JOIN accounts a ON a.id=o.account_id LEFT JOIN categories c ON c.id=o.category_id`

func scanOccurrence(s interface{ Scan(...any) error }) (domain.TransactionOccurrence, error) {
	var x domain.TransactionOccurrence
	err := s.Scan(&x.ID, &x.RecurrenceRuleID, &x.AccountID, &x.AccountName, &x.Kind, &x.CategoryID, &x.CategoryName, &x.SubcategoryID, &x.AmountCents, &x.Description, &x.ScheduledDate, &x.Status, &x.TransactionID, &x.InstallmentNumber, &x.InstallmentCount, &x.CreatedAt, &x.UpdatedAt)
	return x, err
}
...
func (q *dbQueries) SetTransactionRecurrence(ctx context.Context, transactionID, ruleID string) error {
	_, err := q.q.ExecContext(ctx, `UPDATE transactions SET recurrence_rule_id=? WHERE id=?`, ruleID, transactionID)
	return err
}
